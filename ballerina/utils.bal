...
# Merges the internally-managed Process Manager workspace session headers
# (`Cookie`, `X-Signavio-ID`) with the caller-supplied headers for a request.
#
# + sessionHeaders - The session headers established during `Client.init()`
# + extra - The caller-supplied headers for this request
# + return - The combined header map
isolated function withSessionHeaders(map<string> sessionHeaders, map<string|string[]> extra) returns map<string|string[]> {
    map<string|string[]> merged = {};
    foreach [string, string] [name, value] in sessionHeaders.entries() {
        merged[name] = value;
    }
