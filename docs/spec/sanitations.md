...
- **Follow-up (request payload annotations)**: Re-exporting the request types as aliases has a subtle consequence - a value the caller constructs against a re-exported alias (e.g. `signavio:BpmnImportRequest`) carries the *alias* inherent type, and `jsondata:toJson` then does **not** apply the fields' `@jsondata:Name` wire-name mappings (it emits the Ballerina field name, e.g. `bpmn2File` instead of `bpmn2_0file`). This silently broke `importBpmn20Xml` against the live server (the workspace host rejected the upload with `BPMN2_0Import.NoFile` because the file part was misnamed). Fixed by restamping every request payload to its concrete `oas` type via `check payload.cloneWithType(<Type>)` before `jsondata:toJson` in each `oas` operation, which restores the annotations. The mock's `/p/bpmn2_0-import` resource now also asserts the file part is named `bpmn2_0file`, so this regression is caught by `testImportBpmnXml`. Verified fixed against the live au tenant.
