...
14. Wrapped the generated client to add transparent token/session refresh (single-module wrapper, `ballerinax/ardoq` layout)
- **Original**: The generated client class was the public API directly. `Client.init()` obtained the gateway JWT and the workspace session once and held them for the client's lifetime, baking the JWT into `gatewayClient`'s config. A client instance living past the gateway JWT's ~24h TTL (confirmed by decoding the token's `exp` claim against a live tenant) or the workspace session's server-side timeout would then fail every subsequent call with an auth error until re-initialized - a real reliability gap for long-lived consumers (e.g. an HTTP service that constructs the client once at startup).
- **Updated**: Adopted the `ballerinax/ardoq` layout - a single flat module with the generated client kept alongside a hand-written wrapper (no submodule, no type aliases):
  - `oas_client.bal` holds the generated client as an internal `isolated client class GeneratedClient`. It keeps the gateway JWT and workspace session in `lock`-guarded mutable state instead of baking them into the http clients: `gatewayClient` is built plain and the JWT is injected per request (`gatewayHeaders`), the session headers are read per request (`currentSessionHeaders`), and a public `reauthenticate()` re-runs both logins (`POST /auth/v1/token` and `POST /p/login`) and atomically swaps in the fresh state.
  - `client.bal` holds the public `Client`, which wraps `final GeneratedClient oasClient` and delegates each of the 92 operations to it; when a call comes back HTTP 401 it calls `reauthenticate()` once and replays the request. Operation doc comments are carried onto the wrapper so the public API is documented.
  - `types.bal` and `utils.bal` are the real generated types/helpers, shared by both classes in the one module.
- **Reason**: A pure-Ballerina *transparent* wrapper (one that passes the caller's expected type straight through to the HTTP client) is impossible - a dependently-typed function must have an `external`/Java body - so the SAP Business One approach (`b1_http_client.bal`) would require a Java native module. Rebuilding the inner client on 401 was also ruled out because `ConnectionConfig` embeds `http:*` config records and is not `Cloneable`, so it cannot be stored for reconstruction. Injecting refreshable auth per request inside `GeneratedClient` and retrying on 401 in the wrapper closes the expiry gap with neither Java nor a config-storage workaround. Covered by the `testReauthenticatesOn401` mock test (a sentinel request is rejected with 401 exactly once, and the wrapper must re-login and replay to succeed). An earlier attempt used a `modules/oas` submodule with the public module re-exporting the 243 types as aliases; that was abandoned because a type alias (`public type X oas:X;`) drops field-level `@jsondata:Name` annotations for request payloads built against it (`jsondata:toJson` emitted `bpmn2File` instead of `bpmn2_0file`, breaking `importBpmn20Xml` live with `BPMN2_0Import.NoFile`) and also prevents `bal doc` from documenting the types' fields. The single-module layout with real types avoids both problems. The mock's `/p/bpmn2_0-import` resource asserts the file part is named `bpmn2_0file`, so the wire-name regression stays caught by `testImportBpmnXml` (verified against the live au tenant).

Regenerating this connector from `docs/spec/aligned_ballerina_openapi.json` would discard the hand-written wrapper (`client.bal`) and the `reauthenticate`/per-request-auth changes in `oas_client.bal`, along with sanitations 2-13, since they go beyond what `bal openapi` can express (a login handshake isn't representable as an OpenAPI security scheme, and the multipart field-name/extra-field fixes and the `createBodyParts` byte-array bug are in hand-maintained code, not the spec). The spec itself should be corrected (split the single server into the two real backends, drop `ApiKeysConfig`/browser-cookie security schemes, fix the `/spm/v1` paths, the doubled `/v1/objectives` segment, the dictionary-entry response shapes, and the `bpmn2_file`/`bpmn2_0file` field name) before the next regeneration, and these hand-changes reconciled against the regenerated output.
